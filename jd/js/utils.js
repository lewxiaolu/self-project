...
var utils = (function () {
    ...
    /**
     * hover的时候显示 移除隐藏以及类的样式的改变
     * @param curEle 当前元素
     * @param curBottom 要显示的元素
     * @param nowClass 显示的样式
     */
    function hoverAll(curEle,curBottom,nowClass){
        nowClass = nowClass || null;
        curEle.onmouseenter = function (e) {
            curBottom.style.display = 'block';
            if(nowClass){
                this.addClass(curEle, nowClass);
            }
        }
        curEle.onmouseleave= function (e) {
            curBottom.style.display = 'none';
            if(nowClass){
                this.removeClass(curEle, nowClass);
            }
        }
    }
    ...
    /**
     *
     * @param aInput 选项卡上面的按钮
     * @param aDiv 选项卡的内容
     * @param aInputClass 单机按钮显示的类名
     * @param aDivClass 对应按钮让内容现实的类名
     * @param startIndex 默认让那个显示
     */
    function tab(aInput,aDiv,aInputClass,aDivClass,startIndex){
        var _this = this;
        var light=startIndex||0;
        for(var i=0; i<aInput.length; i++){
            this.removeClass(aInput[i],aInputClass);
            this.removeClass(aDiv[i],aDivClass);
        }
        this.addClass(aInput[light],aInputClass);
        this.addClass(aDiv[light],aDivClass);
        for(i=0; i<aInput.length; i++){
            (function(index){
                aInput[index].onmouseover=function(){
                    for(var i=0; i<aInput.length; i++){
                        _this.removeClass(aInput[i],aInputClass);
                        _this.removeClass(aDiv[i],aDivClass);
                    }

                    _this.addClass(aInput[index],aInputClass);
                    _this.addClass(aDiv[index],aDivClass);
                }
            })(i);
        }
    }
