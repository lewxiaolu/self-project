...
AutoBanner.prototype={
    ...
    bind:function bind(){
        ...
        for(var i=0; i<this.data.length; i++){
            ...
            str2+=i==0?'<li class="bg">'+num+'</li>':'<li>'+(num+i)+'</li>';
        }
...
function AutoBanner1(id,ajaxUrl,interval){
...
AutoBanner1.prototype={
    ...
    init:function(){
        ...
        //4.图片自动轮播
        clearInterval(_this.autoTimer);
        this.autoTimer=setInterval(function(){//匿名函数执行不需要主体
            _this.autoMove();//此时里面的主体是_this
        },_this.interval);
        // this.autoTimer=setInterval( _this.autoMove,_this.interval);这种形式autoMove执行没有主体 放的是地址 所以里面的this还是window
        //5.焦点自动轮播
        this.bannerTip();
        //6.鼠标移入停止，移出继续
        this.stopStart();
    ...
    autoMove:function autoMove(){
        ...
        this.step++;
        zhufengAnimate(this.oImgWrap,{'left':-this.step*this.aDiv[0].offsetWidth},500);
        this.bannerTip();
    },
    bannerTip:function bannerTip(){
    ...
    handleChange:function handleChange(){
        ...
        for(var i=0; i<_this.aLi.length; i++){
            ...
            curLi.onclick=function(){
                _this.step=this.index;
                zhufengAnimate(_this.oImgWrap,{'left':-_this.step*_this.aDiv[0].offsetWidth},500,1);
                _this.bannerTip();
            }
        }
    },
    leftRight:function leftRight(){
        ...
        _this.oBtnLeft.onclick=function(){
            ...
            _this.step--;
            zhufengAnimate(_this.oImgWrap,{'left':-_this.step*_this.aDiv[0].offsetWidth},500);
            _this.bannerTip();
        };
    }
